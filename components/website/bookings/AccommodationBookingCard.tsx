...
import { useInitiateAccommodationBookingPayment } from "@/services/explorer/booking";
...
const AccommodationBookingCard: React.FC<AccommodationBookingCardProps> = ({
  booking,
  isSelected = false,
}) => {
  const { mutate: initiatePayment } =
    useInitiateAccommodationBookingPayment();
  const { mutate: verifyPayment } =
    useVerifyPayment();

  // Extract accommodation-specific details
  const accommodationDetails = booking.details as {
    checkInDate: string;
    checkOutDate: string;
    guests: number;
    roomType?: string;
  ...
  // Get accommodation type badge color
      ...
      },
      {
        onSuccess: (response) => {
          ...
          popup.resumeTransaction(
            response?.data?.payment?.access_code,
            {
              ...
              onCancel: () => {
                toast.error(
                  "Payment cancelled. Your booking is still pending."
                );
              },
              onClose: () => {
                toast.error("Payment closed. Your booking is still pending.");
              },
            }
          );
        },
      ...
      <div className="flex flex-col sm:flex-row items-start gap-4">
        ...
        <div className="relative flex-shrink-0 size-14 rounded-lg overflow-hidden">
          <Image
            ...
            alt={booking?.listing?.title || "Accommodation"}
        ...
        <div className="flex-1 w-full">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2 mt-2.5">
                <span
                  className={`capitalize text-[11px] font-medium rounded-sm px-3 py-1 ${getAccommodationTypeColor(
                    booking?.listing?.accommodationType || ""
                  )}`}
                >
                  {booking?.listing?.accommodationType}
                </span>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-6 gap-4">
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs font-medium text-gray-800">
                  {formatDate(accommodationDetails.checkInDate)}
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs font-medium text-gray-800">
                  {formatDate(accommodationDetails.checkOutDate)}
                </p>
              </div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs font-medium text-gray-800">
                  {accommodationDetails.guests}{" "}
                  {accommodationDetails.guests === 1 ? "guest" : "guests"}
                </p>
              </div>
          ...
          {/* Payment and Vendor Approval Status */}
          {/* {(booking?.payment || booking?.vendorApproval) && (
          ...
          <div className="mt-5 flex items-center justify-between border-t border-dashed border-gray-300 pt-5">
            <div className="flex items-center gap-2">
              {/* <Link
                href="/"
                className="flex items-center gap-2 text-primary text-xs font-medium rounded-lg px-4 py-2 border border-primary transition-colors duration-200 hover:bg-primary hover:text-white"
              ...
              {booking?.status === "confirmed" && (
                <Button className="bg-white text-primary text-xs border border-primary shadow-none cursor-pointer hover:bg-primary hover:text-white transition-colors ease-in-out duration-300">
              ...
              {(booking?.status === "pending" ||
                ...
                <Button
                  ...
                  className="bg-white text-primary text-xs border border-primary shadow-none cursor-pointer hover:bg-primary hover:text-white transition-colors ease-in-out duration-300 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  Complete Booking
                </Button>
              )}
...
export default AccommodationBookingCard;