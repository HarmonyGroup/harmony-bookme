      ...
      <TableBody>
        {data?.data?.map((user: User) => (
          <TableRow key={user._id}>
            ...
            <TableCell className="text-gray-500 text-xs font-medium">
              {user?.lastLogin ? moment(user.lastLogin).format("lll") : "No session"}
            </TableCell>
  ...
  const renderEmptyState = () => (
    <div className="flex flex-col items-center justify-center gap-2 py-20 min-h-[400px]">
      <Image
        ...
        loading="lazy"
      />
      <h1 className="text-gray-700 text-sm font-semibold">No explorer</h1>
      <p className="text-gray-500 text-xs text-center max-w-md">
        Registered explorers will show up here
      </p>
    </div>
  );
